...
import { install, uninstall, setEnabled, setMode } from '../lib/installer.js';
import { ensureProjectFiles, getProjectRoot, getPaths, loadConfig } from '../lib/project.js';
...
async function main() {
  ...
  try {
    switch (cmd) {
      ...
      case 'uninstall': return cmdUninstall(projectRoot);
...
function cmdReport(projectRoot) {
  ...
  console.log(`- ${html}`);
  console.log(`- ${svg}`);
  console.log(`Estimated tokens saved this week: ${Math.round(model.tokensSaved).toLocaleString('en-US')}`);
}
