...
export function summarizeSavings(projectRoot = process.cwd(), options = {}) {
  ...
  for (const event of events) {
    ...
    if (event.type === 'read_guard_block') {
      const tokens = number(event.estimatedTokens || event.tokens);
    ...
    if (event.type === 'bash_output_trimmed') {
      const original = number(event.originalTokens);
      const trimmed = number(event.trimmedTokens);
    ...
    if (event.type === 'long_input_digest_written') {
      const original = number(event.originalTokens || event.estimatedTokens);
      const digest = number(event.digestTokens);
      const saved = Math.max(0, original - digest);

      model.longInputDigests += 1;
      model.longInputDigestSavedTokens += saved;

      // Long input digest usually saves follow-up turns, not necessarily the current turn.
      // Count conservatively in net to avoid exaggerated reports.
    ...
    if (event.type === 'command_cache_hit') {
      const saved = number(event.savedTokens || event.estimatedTokens);
    ...
    if (event.type === 'handoff_compressed') {
      const saved = number(event.savedTokens);
    ...
    if (event.type === 'token_guard_overhead') {
      const overhead = number(event.tokens || event.estimatedTokens);
...
function addTopWaster(map, file, tokens) {
  ...
  map.set(file, (map.get(file) || 0) + number(tokens));
}
