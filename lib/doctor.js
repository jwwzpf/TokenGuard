...
export function runDoctor(projectRoot = process.cwd()) {
  ...
  add(
    'mixed-version-dirs',
    'Mixed old/new local folders',
    legacyExists ? 'warn' : 'pass',
    legacyExists
      ? 'Legacy .token-guard/ folder detected alongside current TokenGuard/ support.'
      : 'No legacy .token-guard/ folder detected.',
    'Run `token-guard uninstall`, then reinstall with `token-guard install --observe`.'
  );

  add(
    'legacy-python-mvp',
    'Legacy Python MVP file',
    legacyPythonExists ? 'warn' : 'pass',
    legacyPythonExists
      ? 'Legacy token_guard.py detected.'
      : 'No legacy token_guard.py detected.',
    'Run `token-guard uninstall`, then reinstall with `token-guard install --observe`.'
  );
  ...
  if (config) {
    ...
    add(
      ...
      Number(config.thresholds?.softTokens) === 25000 &&
        ...
        Number(config.thresholds?.narrowReadMaxLines) === 200
        ? 'pass'
        : 'warn',
      `soft=${config.thresholds?.softTokens}, hard=${config.thresholds?.hardTokens}, narrowReadMaxLines=${config.thresholds?.narrowReadMaxLines}`,
      'Recommended v0.3 defaults are soft=25000, hard=60000, narrowReadMaxLines=200.'
    );
    ...
    add(
      ...
      missingAllow.length === 0
        ? 'Token Guard self files and summaries are allowlisted.'
  ...
  add(
    ...
    'Local-first posture',
    'pass',
    'No daemon, no cloud backend, no API key, no upload path is required by Token Guard.'
  );
...
function checkClaudeSettings(projectRoot, paths, add) {
  ...
  const oldCommands = commands.filter(command =>
    isTokenGuardCommand(command) && !command.includes('token-guard hook')
  );
  ...
  add(
    ...
    tokenGuardCommands.length
      ? `Found ${tokenGuardCommands.length} current token-guard hook command(s).`
      : 'No current token-guard hook commands found.',
    'Run `token-guard install --observe`.'
  );

  add(
    'legacy-hook-commands',
    'Legacy Token Guard hook commands',
    oldCommands.length === 0 ? 'pass' : 'warn',
    oldCommands.length
      ? `Found legacy hook command(s): ${oldCommands.join(' | ')}`
      : 'No legacy Token Guard hook commands detected.',
    'Run `token-guard uninstall`, then reinstall with `token-guard install --observe`.'
  );
}
