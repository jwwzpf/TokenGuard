...
import { getCodexActivity } from './session-check.js';
...
export function runDoctor(projectRoot = process.cwd()) {
  ...
  checkCodexChannel(projectRoot, paths, add);
...
function checkCodexChannel(projectRoot, paths, add) {
  if (!fs.existsSync(paths.agents)) {
    add('codex-channel','Codex active CLI usage','warn','AGENTS.md not installed — Codex channel inactive.','Run `token-guard install` (without --no-codex).');
    return;
  }

  const activity = getCodexActivity(projectRoot);

  if (!activity.present) {
    add('codex-channel','Codex active CLI usage','warn','No `tg turn-tick` recorded yet. Codex may not be calling Token Guard CLI per AGENTS.md rules.','Inside Codex: ensure each turn ends with `tg turn-tick --note "<short goal>"` and start with `tg session-check`. Re-run `token-guard install` to refresh AGENTS.md rules.');
    return;
  }

  if (!activity.lastTickAt) {
    add('codex-channel','Codex active CLI usage','pass',`Session marker present, no ticks yet in current session (turn 0). Cumulative: 0 tok.`);
    return;
  }

  const lastTickMs = new Date(activity.lastTickAt).getTime();
  const ageHours = (Date.now() - lastTickMs) / (1000 * 60 * 60);

  if (ageHours > 48) {
    add('codex-channel','Codex active CLI usage','warn',`Last turn-tick was ${Math.round(ageHours)}h ago. Codex may have stopped calling Token Guard.`,'Confirm Codex follows AGENTS.md rules. Run `token-guard upgrade` if AGENTS.md is out of date.');
    return;
  }

  add('codex-channel','Codex active CLI usage','pass',`Last tick ${activity.lastTickAt} · ${activity.turnCount} turns · ~${Number(activity.cumulative).toLocaleString('en-US')} tok cumulative.`);
}
