...
function onPostToolUse(projectRoot, config, input) {
  ...
  appendEvent(projectRoot, {
    ...
    savedTokens: Math.max(0, Number(result.originalTokens || 0) - Number(result.trimmedTokens || 0)),
  ...
  const saved = Math.max(0, Number(result.originalTokens || 0) - Number(result.trimmedTokens || 0));
