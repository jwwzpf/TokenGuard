...
import { buildRoutingHint, classifyPrompt, recordSubagentDelegation } from './model-router.js';
import {
...
function onUserPromptSubmit(projectRoot, config, input) {
  ...
  const routingClass = classifyPrompt(prompt);
  ...
  } else if (!routingHint && routingClass === 'ambiguous' && prompt.length >= Number(config.modelRouter?.minPromptCharsForHint || 600)) {
    appendEvent(projectRoot, {
      type: 'model_route_hint_suppressed',
      reason: 'ambiguous_quality_first',
      promptChars: prompt.length
    });
  }
...
function onPostAgent(projectRoot, config, input) {
  ...
  const isCheaperChild = record.childModel !== record.parentModel && record.savingsFactor > 0;
  const isExplorePattern = /explore/i.test(String(record.subagentType));
  const isOpusParent = /opus/i.test(String(record.parentModel));
  const isGoodPattern = isExplorePattern && isOpusParent && isCheaperChild;

  if (isGoodPattern) {
    appendEvent(projectRoot, {
      type: 'good_pattern_observed',
      pattern: 'opus_main_axis_with_explore_subagent',
      parentModel: record.parentModel,
      childModel: record.childModel,
      savedEquivalentTokens: record.savedEquivalentTokens
    });
  ...
  const verifyLine = isCheaperChild
    ? `- Verify subagent claims (class names, file paths, line numbers) before depending on them — cheaper models can hallucinate plausibly.`
    : '- Subagent ran on same tier as parent (no model savings). Output verification still recommended.';

  const lines = [
    `Token Guard subagent delegation tracked:`,
    `- subagent_type=${record.subagentType} · child=${record.childModel} · parent=${record.parentModel}`,
    `- estimated savings: ~${format(record.savedEquivalentTokens)} ${record.parentModel}-equivalent tokens (factor ${record.savingsFactor})`
  ];

  if (isGoodPattern) {
    lines.push('- Good pattern: parent main-axis + Explore subagent for info gathering. Safe + cheap. Keep using this for heavy tasks.');
  }
