...
function onUserPromptSubmit(projectRoot, config, input) {
  ...
  const message =
    config.mode === 'observe'
      ? 'Token Guard observe mode is active. It records token waste and gives targeted guidance, but it will not block reads or bypass normal permissions. Full-read escape hatch: @tg:force-read <file> or @tg:allow-full-read.'
...
function onPreRead(projectRoot, config, input) {
  ...
  if (consumeForceRead(projectRoot, config, info.relPath)) {
    ...
    return preToolDecision(
      ...
      `Force-read was used for ${info.relPath}. This was a one-time override.`
    );
  }
  ...
  if (config.mode === 'observe') {
    ...
    return observeNotice(
      'PreToolUse',
      `Token Guard observe notice: ${message} This is observe mode, so the read is not blocked and normal Claude permissions are not bypassed.`
    );
  }
  ...
  if (info.tokens >= hard || info.alwaysGuarded || info.looksLog || config.mode === 'strict') {
    ...
    return preToolDecision('deny', message, message);
  }
  ...
  return preToolDecision('ask', message, message);
}
...
function onPreBash(projectRoot, config, input) {
  ...
  if (config.mode === 'observe') {
    return observeNotice(
      'PreToolUse',
      `Token Guard observe notice: ${message} This is observe mode, so the command is not blocked and normal Claude permissions are not bypassed.`
    );
  }
...
function onPostToolUse(projectRoot, config, input) {
  ...
  const additionalContext =
    ...
    'Head/tail and error/failure lines were preserved when detected.';
  ...
  return {
    ...
    // Kept for compatibility with older Claude Code hook JSON examples.
    hookSpecificOutput: {
