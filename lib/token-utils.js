...
export function estimateTokens(text = '') {
  ...
  // Conservative approximation for source/log text. Exact tokenization is model-dependent.
...
export function looksTextFile(file = '') {
  ...
  if (
    /\.(js|jsx|ts|tsx|mjs|cjs|json|md|mdx|txt|yml|yaml|toml|xml|html|css|scss|sass|less|py|rb|php|java|kt|kts|scala|go|rs|c|h|cpp|hpp|cs|swift|dart|sh|bash|zsh|fish|sql|graphql|gql|env|properties|gradle|pom|lock|log)$/i.test(p)
  ) {
    return true;
  }
...
export function fileInfo(projectRoot = process.cwd(), filePath = '', config = {}) {
  ...
  if (exists) {
    try {
      ...
      if (stat.isFile() && looksTextFile(absolute)) {
...
export function summarizeCommandOutput(output = '', options = {}) {
  ...
  const maxLines = Number(options.maxLines || 140);
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (/(error|failed|failure|exception|traceback|assert|expected|received|panic|fatal|timeout|exit code|denied|BUILD FAILURE|BUILD SUCCESS|Tests run:|Failed tests?:|Caused by:)/i.test(line)) {
      addAround(keep, i, lines.length, 2);
    }

    if (/^@@\s+[-+0-9, ]+@@/.test(line) || /^diff --git /.test(line) || /^[+-](?![+-])/.test(line)) {
      keep.add(i);
    }
  }
  ...
  let summary = selected.lines.join('\n');

  if (summary.length > maxChars) {
    summary = `${summary.slice(0, maxChars)}\n[Token Guard: command output summary clipped to ${maxChars.toLocaleString('en-US')} chars.]`;
  }
