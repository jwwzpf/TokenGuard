...
function renderHtml(m) {
  ...
  const headline = saved > 0
    ...
    : potential > 0
      ? `${format(potential)} tokens flagged`
      : 'Token Guard is ready';

  const subtitle = saved > 0
    ? 'Net savings after Token Guard overhead.'
    : potential > 0
      ? 'Potential avoidable context detected. Smart Savings only intervenes on high-confidence waste.'
      : 'Run a few coding sessions, then generate this report again.';

  const trend = renderTrendSvg(m.daily || []);
  const ring = renderRingSvg({ saved, gross, overhead });
  const sources = renderSourceBars(m);
  const contextTable = renderTopContextSavings(m.topContextSavings || []);
  const commandTable = renderTopCompressedCommands(m.topCompressedCommands || []);
...
<style>
:root{--bg:#050814;--panel:rgba(15,23,42,.72);--line:rgba(148,163,184,.22);--text:#f8fafc;--muted:#94a3b8;--green:#22c55e;--blue:#38bdf8;--violet:#a78bfa;--amber:#f59e0b;--rose:#fb7185}
...
.hero{position:relative;overflow:hidden;border:1px solid var(--line);border-radius:34px;padding:42px;background:linear-gradient(135deg,rgba(15,23,42,.94),rgba(2,6,23,.72));box-shadow:0 30px 110px rgba(0,0,0,.45)}
.kicker{color:var(--blue);font-size:13px;font-weight:800;letter-spacing:.16em;text-transform:uppercase}
.title{margin:12px 0 4px;font-size:clamp(46px,8vw,92px);line-height:.9;letter-spacing:-.07em;font-weight:950}
.subtitle{font-size:22px;color:#cbd5e1;margin:18px 0 0}
...
.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:20px}
.metric{border:1px solid var(--line);border-radius:22px;background:rgba(15,23,42,.6);padding:18px}
...
.pill{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--line);border-radius:999px;padding:8px 12px;color:#cbd5e1;background:rgba(15,23,42,.55);font-size:13px;margin-top:22px}
...
@media(max-width:850px){.grid,.metrics{grid-template-columns:1fr}.title{font-size:54px}}
</style>
</head>
<body>
<main>
<section class="hero">
  <div class="kicker">Token Guard · Smart Savings</div>
  <div class="title">${escapeHtml(headline)}</div>
  <p class="subtitle">${escapeHtml(subtitle)}</p>
  <div class="pill">Local-first · no daemon · no code upload</div>
  <div class="metrics">
    <div class="metric"><div class="label">Net saved</div><div class="value green">${format(saved)}</div></div>
    <div class="metric"><div class="label">Gross protected</div><div class="value blue">${format(gross)}</div></div>
    <div class="metric"><div class="label">Context reads</div><div class="value amber">${format(m.contextReadSavedTokens || 0)}</div></div>
    <div class="metric"><div class="label">TG overhead</div><div class="value violet">${format(overhead)}</div></div>
  </div>
</section>

<div class="grid">
  <section class="card"><h2>7-day savings trend</h2>${trend}</section>
  <section class="card"><h2>Net after overhead</h2>${ring}</section>
</div>

<div class="grid">
  <section class="card"><h2>Savings sources</h2>${sources}</section>
  <section class="card"><h2>What helped most</h2><p class="insight">${escapeHtml(bestInsight(m))}</p></section>
</div>

<div class="grid">
  <section class="card"><h2>Top context savings</h2>${contextTable}</section>
  <section class="card"><h2>Top compressed commands</h2>${commandTable}</section>
</div>

<div class="footer">Generated by Token Guard · Smart token savings for AI coding agents</div>
</main>
</body>
</html>`;
}
...
function bestInsight(m) {
  const candidates = [
    ['Targeted context reads avoided full-file dumps.', m.contextReadSavedTokens || 0],
    ...
    ['Read guards prevented high-confidence full-context dumps.', m.savingsSources?.readGuards || 0]
  ].sort((a, b) => b[1] - a[1]);

  if ((candidates[0]?.[1] || 0) > 0) {
    return `${candidates[0][0]} Estimated contribution: ${format(candidates[0][1])} tokens.`;
  }

  if ((m.potentialWasteFlaggedTokens || 0) > 0) {
    return `Token Guard found ${format(m.potentialWasteFlaggedTokens)} potential avoidable tokens. It intervenes only on high-confidence waste to protect coding quality.`;
  }
...
function renderTrendSvg(rows) {
  ...
  const pts = data.map((d, i) => {
    ...
    const y = h - p - (Math.max(0, d.net || 0) * (h - p * 2) / max);
...
function renderSourceBars(m) {
  const rows = [
    ['Context reads', m.contextReadSavedTokens || 0, '#38BDF8'],
...
function renderTopContextSavings(rows = []) {
  if (!rows.length) {
    return '<p class="small">No targeted context savings recorded yet. Use <span class="mono">tg ctx</span> or <span class="mono">token-guard read</span> during coding sessions.</p>';
  }
...
function renderShareSvg(m) {
  const saved = Math.max(0, Number(m.netSavingsTokens || 0));
  const contextSaved = Math.max(0, Number(m.contextReadSavedTokens || 0));
  const bashSaved = Math.max(0, Number(m.bashOutputSavedTokens || 0));
  const title = saved > 0 ? `${format(saved)} tokens saved` : 'Token Guard ready';

  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><radialGradient id="a" cx="20%" cy="0%" r="80%"><stop offset="0" stop-color="#38BDF8" stop-opacity=".45"/><stop offset="1" stop-color="#050814"/></radialGradient><radialGradient id="b" cx="90%" cy="20%" r="70%"><stop offset="0" stop-color="#A78BFA" stop-opacity=".35"/><stop offset="1" stop-color="#050814" stop-opacity="0"/></radialGradient></defs><rect width="1200" height="630" fill="#050814"/><rect width="1200" height="630" fill="url(#a)"/><rect width="1200" height="630" fill="url(#b)"/><text x="72" y="105" fill="#38BDF8" font-family="Inter,Arial" font-size="25" font-weight="800" letter-spacing="4">TOKEN GUARD · SMART SAVINGS</text><text x="72" y="250" fill="#F8FAFC" font-family="Inter,Arial" font-size="88" font-weight="900">${escapeSvg(title)}</text><text x="72" y="335" fill="#CBD5E1" font-family="Inter,Arial" font-size="34">Precise context, less terminal noise.</text><rect x="72" y="430" width="318" height="92" rx="26" fill="#0F172A" fill-opacity=".75" stroke="#334155"/><text x="98" y="470" fill="#94A3B8" font-family="Inter,Arial" font-size="19" font-weight="700">Context reads</text><text x="98" y="506" fill="#38BDF8" font-family="Inter,Arial" font-size="31" font-weight="900">${format(contextSaved)}</text><rect x="420" y="430" width="318" height="92" rx="26" fill="#0F172A" fill-opacity=".75" stroke="#334155"/><text x="446" y="470" fill="#94A3B8" font-family="Inter,Arial" font-size="19" font-weight="700">Bash noise</text><text x="446" y="506" fill="#22C55E" font-family="Inter,Arial" font-size="31" font-weight="900">${format(bashSaved)}</text><rect x="768" y="430" width="318" height="92" rx="26" fill="#0F172A" fill-opacity=".75" stroke="#334155"/><text x="794" y="470" fill="#94A3B8" font-family="Inter,Arial" font-size="19" font-weight="700">Local-first</text><text x="794" y="506" fill="#A78BFA" font-family="Inter,Arial" font-size="31" font-weight="900">No upload</text><text x="72" y="590" fill="#94A3B8" font-family="Inter,Arial" font-size="22">Generated by Token Guard</text></svg>`;
}
