...
export function generateReport(projectRoot = process.cwd()) {
  ...
  const money = computeMoney(display.savedTokens, model.grossAvoidedTokens);
  model.money = money;

  const html = renderHtml(model, display, money);
  const svg = renderShareSvg(model, display, money);
...
function computeMoney(savedTokens, grossTokens) {
  ...
  return { sonnetRate, opusRate, usdSonnet, usdOpus, usdGrossOpus, coffees };
}

function renderHtml(model, display, money) {
  ...
  const headlineSub = isSaved
    ? `≈ $${formatMoney(money.usdSonnet)} – $${formatMoney(money.usdOpus)} saved this week`
    : isFlagged
      ? 'tokens protected · awaiting more sessions'
      : 'Run a few coding sessions, then regenerate.';

  const tokenLine = isSaved
    ? `${format(saved)} tokens reclaimed · ${formatCompact(gross)} gross protected`
    : isFlagged
      ? `${format(positive(model.potentialWasteFlaggedTokens))} tokens flagged`
      : 'Local-first · no daemon · no code upload';

  const coffeeLine = isSaved
    ? `That's roughly <b>${formatCoffee(money.coffees)} coffees</b> ☕ at Opus rates — without lifting a finger.`
    : isFlagged
      ? 'Token Guard is watching — it only intervenes on high-confidence waste.'
      : 'Once savings accrue, we\'ll show your coffee equivalent here.';
...
.metric .value{font-size:34px;font-weight:900;margin-top:8px;letter-spacing:-.04em;line-height:1}
...
<main>
  <div class="brandbar">
    ...
    <div>${escapeHtml(dateStamp)} · last 7 days</div>
  </div>
  ...
  <section class="hero">
    <div class="hero-inner">
      ...
      <div class="metrics">
        <div class="metric"><div class="label">USD saved · Opus</div><div class="value gold">$${formatMoney(money.usdOpus)}</div><div class="sub">@ $${money.opusRate}/M input</div></div>
        <div class="metric"><div class="label">USD saved · Sonnet</div><div class="value green">$${formatMoney(money.usdSonnet)}</div><div class="sub">@ $${money.sonnetRate}/M input</div></div>
        <div class="metric"><div class="label">Net tokens</div><div class="value blue">${formatCompact(saved)}</div><div class="sub">after overhead</div></div>
        <div class="metric"><div class="label">Gross protected</div><div class="value violet">${formatCompact(gross)}</div><div class="sub">before overhead</div></div>
      </div>
      ${badges}
    </div>
  </section>
  ...
  <div class="grid-2">
    <section class="card">
      <h2><span class="dot"></span>What happened</h2>
      ${proofHtml}
    </section>
    <section class="card">
  ...
  <div class="grid-2">
    <section class="card">
      <h2><span class="dot"></span>Top context wins</h2>
      ${topContext}
    </section>
    <section class="card">
      <h2><span class="dot"></span>Top compressed commands</h2>
      ${topCommands}
    </section>
  </div>
...
function renderBadges(model, display, money) {
  ...
  if (positive(model.bashOutputSavedTokens) >= 10_000) list.push(['violet', '🧹', 'Bash noise tamed']);
  if (positive(model.longInputDigests) >= 5) list.push(['violet', '📚', `${format(model.longInputDigests)} long inputs digested`]);
  if (positive(model.commandCacheHits) >= 10) list.push(['', '⚡', `${format(model.commandCacheHits)} cache hits`]);
  if (positive(model.handoffCompressedSavedTokens) >= 5_000) list.push(['violet', '🤝', 'Handoff compressed']);
...
function renderProofPoints(rows) {
  if (!rows.length) {
    return '<p class="small">No savings event has been recorded yet. Run a few coding sessions, use <code>tg ctx</code>, or run tests/builds with noisy output, then generate this report again.</p>';
  }

  return `<div class="proof">${rows.slice(0, 6).map(([label, value]) => `<div class="proof-item"><span>${escapeHtml(label)}</span><b class="${value < 0 ? 'minus' : ''}">${value >= 0 ? '+' : '−'}${format(Math.abs(value))}</b></div>`).join('')}</div>`;
}
...
function renderTopContextSavings(rows = []) {
  if (!rows.length) {
    return '<p class="small">No targeted context savings recorded yet. Use <code>tg ctx</code> during coding sessions.</p>';
  }

  return `<div class="table">${rows.slice(0, 6).map(row => `<div class="row"><div><div class="mono">${escapeHtml(shorten(row.file, 52))}</div><div class="small">${escapeHtml(row.method || 'ctx')} · ${format(row.reads || 0)} read(s)</div></div><strong>${format(row.savedTokens)}</strong></div>`).join('')}</div>`;
}
...
function renderTopCompressedCommands(rows = []) {
  ...
  return `<div class="table">${rows.slice(0, 6).map(row => `<div class="row"><div><div class="mono">${escapeHtml(shorten(row.command, 52))}</div><div class="small">${escapeHtml(row.profile || 'tool')} · ${format(row.runs || 0)} run(s)</div></div><strong>${format(row.savedTokens)}</strong></div>`).join('')}</div>`;
}

function renderShareSvg(model, display, money) {
  const saved = positive(display.savedTokens);
  const gross = positive(model.grossAvoidedTokens);
  const isSaved = display.mode === 'saved';
  const isFlagged = display.mode === 'potential';
  ...
  const subtitle = isSaved
    ? `${formatCompact(saved)} tokens reclaimed · last 7 days`
    : isFlagged
      ? 'tokens flagged · awaiting more sessions'
      : 'Token Guard is watching';
  const coffeeText = isSaved ? `≈ ${formatCoffee(money.coffees)} coffees ☕` : 'Local-first · zero upload';
  const titleGrad = isSaved ? 'url(#hl)' : 'url(#hlAmber)';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
    <defs>
      <radialGradient id="bg1" cx="15%" cy="0%" r="80%"><stop offset="0" stop-color="#38BDF8" stop-opacity=".50"/><stop offset="1" stop-color="#04060F" stop-opacity="0"/></radialGradient>
      <radialGradient id="bg2" cx="92%" cy="20%" r="70%"><stop offset="0" stop-color="#A78BFA" stop-opacity=".42"/><stop offset="1" stop-color="#04060F" stop-opacity="0"/></radialGradient>
      <radialGradient id="bg3" cx="50%" cy="110%" r="80%"><stop offset="0" stop-color="#22C55E" stop-opacity=".38"/><stop offset="1" stop-color="#04060F" stop-opacity="0"/></radialGradient>
    ...
    <g transform="translate(72,88)">
      ...
      <text x="22" y="11" fill="#CBD5E1" font-family="Inter,Arial" font-size="20" font-weight="800" letter-spacing="3">TOKEN GUARD · WEEKLY SAVINGS</text>
    </g>

    <g filter="url(#softGlow)" opacity=".55">
      <text x="72" y="290" font-family="Inter,Arial" font-size="180" font-weight="900" fill="${titleGrad}" letter-spacing="-9">${escapeSvg(titleUsd)}</text>
    </g>
    <text x="72" y="290" font-family="Inter,Arial" font-size="180" font-weight="900" fill="${titleGrad}" letter-spacing="-9">${escapeSvg(titleUsd)}</text>

    <text x="72" y="350" fill="#F8FAFC" font-family="Inter,Arial" font-size="32" font-weight="700">${isSaved ? 'saved this week with Token Guard' : 'Token Guard · local-first'}</text>
    <text x="72" y="395" fill="#CBD5E1" font-family="Inter,Arial" font-size="22">${escapeSvg(subtitle)}</text>
    <text x="72" y="430" fill="#94A3B8" font-family="Inter,Arial" font-size="20">${escapeSvg(coffeeText)}</text>

    <g transform="translate(72,470)">
      <rect width="320" height="92" rx="22" fill="#0F172A" fill-opacity=".82" stroke="#334155"/>
      <text x="22" y="36" fill="#94A3B8" font-family="Inter,Arial" font-size="14" font-weight="800" letter-spacing="2">SONNET RATE</text>
      <text x="22" y="74" fill="#4ADE80" font-family="Inter,Arial" font-size="34" font-weight="900">$${escapeSvg(formatMoney(money.usdSonnet))}</text>
    </g>
    <g transform="translate(412,470)">
      <rect width="320" height="92" rx="22" fill="#0F172A" fill-opacity=".82" stroke="#334155"/>
      <text x="22" y="36" fill="#94A3B8" font-family="Inter,Arial" font-size="14" font-weight="800" letter-spacing="2">OPUS RATE</text>
      <text x="22" y="74" fill="#FBBF24" font-family="Inter,Arial" font-size="34" font-weight="900">$${escapeSvg(formatMoney(money.usdOpus))}</text>
    </g>
    <g transform="translate(752,470)">
      <rect width="376" height="92" rx="22" fill="#0F172A" fill-opacity=".82" stroke="#334155"/>
      <text x="22" y="36" fill="#94A3B8" font-family="Inter,Arial" font-size="14" font-weight="800" letter-spacing="2">PROTECTED</text>
      <text x="22" y="74" fill="#38BDF8" font-family="Inter,Arial" font-size="34" font-weight="900">${escapeSvg(formatCompact(gross))} tokens</text>
    </g>
