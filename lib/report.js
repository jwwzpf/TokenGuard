...
function renderShareSvg(model, display, money, todayDisplay = { savedTokens: 0 }) {
  ...
  const anyEquiv = (Number(money.coffees) + Number(money.novels) + Number(money.pizzas) + Number(money.movies)) > 0;
  const equivItems = anyEquiv
    ? [
        ['☕', formatEquivalent(money.coffees), 'COFFEES'],
        ['📖', formatEquivalent(money.novels), 'NOVELS'],
        ['🍕', formatEquivalent(money.pizzas), 'PIZZAS'],
  ...
  const svgGenStamp = new Date().toISOString();
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1200" height="630" viewBox="0 0 1200 630">
    <!-- generated ${svgGenStamp} -->
    <defs>
    ...
    <text x="72" y="618" font-family="Inter,Arial" font-size="17" font-weight="600"><tspan fill="#CBD5E1">Local-first · zero upload · </tspan><a xlink:href="https://github.com/jwwzpf/TokenGuard" target="_blank" rel="noopener"><tspan fill="#A78BFA" text-decoration="underline">github.com/jwwzpf/TokenGuard</tspan></a></text>
  </svg>`;
}
