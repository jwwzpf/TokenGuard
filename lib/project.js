...
export function ensureProjectFiles(projectRoot = process.cwd()) {
  const paths = getPaths(projectRoot);
  for (const dir of [paths.base, paths.reports, paths.memory, paths.sessions, paths.summaries, paths.ledger]) ensureDir(dir);
  ...
  if (!fs.existsSync(paths.decisions)) fs.writeFileSync(paths.decisions, '# Decisions\n\n');
  if (!fs.existsSync(paths.gotchas)) fs.writeFileSync(paths.gotchas, '# Gotchas\n\n');
  if (!fs.existsSync(paths.handoff)) {
    fs.writeFileSync(paths.handoff, '# Next Session Handoff\n\n## Current Goal\n\n## What Changed\n\n## Files Touched\n\n## Tests Run\n\n## Current Blocker\n\n## Next Smallest Task\n\n## Do Not Re-investigate\n\n');
  }
  if (!fs.existsSync(paths.ledgerEvents)) fs.writeFileSync(paths.ledgerEvents, '');
...
function migrateLegacy(projectRoot) {
  ...
  try {
    fs.cpSync(paths.legacyBase, paths.base, { recursive: true, force: false });
  } catch {
...
export function updateGitignore(projectRoot = process.cwd()) {
  ...
  let content = fs.existsSync(paths.gitignore) ? fs.readFileSync(paths.gitignore, 'utf8') : '';
  let changed = false;
  ...
  if (changed) fs.writeFileSync(paths.gitignore, content);
}
