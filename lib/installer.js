...
export function upgrade(projectRoot = process.cwd(), options = {}) {
  const before = getPaths(projectRoot);
  const hadTokenGuardData = fs.existsSync(before.base);
  const hadClaudeSettings = fs.existsSync(before.claudeSettingsLocal);
  const hadAgents = fs.existsSync(before.agents);
  const hadClaudeLocal = fs.existsSync(before.claudeLocal);

  const result = install(projectRoot, options);

  return {
    ...result,
    upgraded: true,
    preservedData: hadTokenGuardData,
    refreshed: {
      config: true,
      claudeHooks: options.claude !== false,
      codexInstructions: options.codex !== false,
      claudeLocal: true,
      gitignore: true
    },
    previous: {
...
## Session monitoring (Codex-specific) — REQUIRED, not optional

Codex has no native hooks. Without these CLI calls, Token Guard cannot observe context pressure and cannot warn you when you should switch sessions. **You MUST run them every turn.**

Mandatory loop (treat as part of your work, not as overhead):

1. **First turn of a fresh Codex session**: run \`tg session-check --reset\` once. This marks a session boundary so cumulative-token counting restarts.
2. **Start of every turn**: run \`tg session-check\`.
   - Exit 0 + \`OK\` → continue normally.
   - Exit 0 + \`WARN\` → prefer narrow Read / \`tg ctx\`; do not load new long files; plan a handoff in the next 1–2 turns.
   - **Exit 2 + \`SWITCH\`** → STOP loading new context. Finish the current atomic sub-step only. Then run \`tg handoff write\` and tell the user: "Context budget reached. Recommend new Codex session — next session will SessionStart-load TokenGuard/sessions/handoff.md."
3. **End of every turn**: run \`tg turn-tick --output-tokens <approx-of-this-reply> --tool "<tools-used>" --note "<one-line goal>"\`.
   - \`--output-tokens\` may be approximate (use char-count/4 as a rough estimate).
   - You can also pass \`--prompt "<user-message-text>"\` and Token Guard will estimate prompt tokens for you.
4. **Before \`/clear\` or starting a fresh session**: run \`tg handoff write --goal "<current goal>" --note "<what's next>"\`. The next session's SessionStart-equivalent auto-loads \`TokenGuard/sessions/handoff.md\`.

Inline pressure warnings: every \`tg ctx\` / \`tg read\` / \`tg find\` / \`tg summarize\` / \`tg edit\` output ends with a session-pressure footer if WARN or SWITCH. If you see one, act on it immediately — do not wait for the next \`tg session-check\`.

Why this matters: without these calls Token Guard's stats look conservative or empty and you will not get a SWITCH alert before exceeding model context. These commands are local, fast, and non-blocking. They never upload anything.

Concrete example of one turn:

\`\`\`
$ tg session-check
Token Guard session-check: ✓ OK
Cumulative: ~24,300 tokens · Turns: 9
...

# do your work, including narrow reads:
$ tg ctx src/auth.ts --focus verifyToken
Token Guard autopilot context
...
Estimated saved: 5,400 tokens (~92% smaller)

# end of turn:
$ tg turn-tick --output-tokens 420 --tool "tg-ctx,Read" --note "trace verifyToken"
Token Guard session-check: ✓ OK
Cumulative: ~30,120 tokens · Turns: 10
\`\`\`
${TG_END}
`;
