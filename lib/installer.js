...
import { ensureDir, ensureProjectFiles, getPaths, loadConfig, saveConfig, updateGitignore } from './project.js';
...
function normalizeMode(mode) {
  ...
  if (!allowed.has(mode)) throw new Error(`Invalid mode: ${mode}. Use observe, active, edit, or strict.`);
  return mode;
}
...
function uninstallClaudeHooks(projectRoot) {
  ...
  for (const eventName of Object.keys(hooks)) {
    ...
    if (Array.isArray(hooks[eventName]) && hooks[eventName].length === 0) delete hooks[eventName];
  }
...
function stripTokenGuardHooks(eventHooks = []) {
  ...
  return eventHooks
    .map(group => ({
      ...group,
      hooks: (group.hooks || []).filter(hook => !String(hook.command || '').includes('token-guard hook'))
    }))
...
function writeClaudeLocal(paths) {
  const section = `${TG_START}\n# Token Guard\n\nToken Guard is installed for this local project. Use \`TokenGuard/memory/core.md\` for durable facts and \`TokenGuard/sessions/handoff.md\` for next-session handoff. Prefer targeted context over broad full-file reads.\n${TG_END}\n`;
...
function upsertMarkedSection(file, section) {
  ...
  if (re.test(content)) content = content.replace(re, section);
