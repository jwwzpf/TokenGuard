...
export function smartRead(projectRoot = process.cwd(), filePath, options = {}) {
  ...
  if (options.diff) {
    result = diffRead(projectRoot, relPath, content, maxTokens);
    return recordTargetedRead(projectRoot, result, {
      method: 'diff',
      query: ''
    });
  }
  ...
  if (options.around) {
    result = readAroundText(
      projectRoot,
      relPath,
      content,
      lines,
      options.around,
      Number(options.context || options.contextLines || 10),
      maxTokens
    );

    return recordTargetedRead(projectRoot, result, {
      method: 'around',
      query: options.around
    });
  }
  ...
  if (options.symbol || options.focus) {
    ...
    if (!match) {
      result = {
        ...
        file: relPath,
        requested,
        text: `Token Guard could not find symbol "${requested}" in ${relPath}.\n\nKnown symbols:\n${symbols.slice(0, 60).map(s => `- ${s.name} (${s.kind}) at line ${s.line}`).join('\n') || '- none detected'}\n`,
  ...
  if (options.section) {
    ...
    if (!section) {
      result = {
        ...
        text: `Token Guard could not find section/key "${options.section}" in ${relPath}.\n\nTip: use --lines A:B, --around TEXT, or --symbol NAME for targeted reads.\n`,
        originalTokens: estimateTokens(content),
        returnedTokens: 0
      };
  ...
  result = makeLargeFilePreview(relPath, content, lines, maxTokens);

  return recordTargetedRead(projectRoot, result, {
    method: 'large-preview',
    query: ''
  });
}
...
export function formatSmartReadResult(result) {
  const saved = Math.max(0, Number(result.originalTokens || 0) - Number(result.returnedTokens || 0));
  ...
  const header = [
    ...
    saved > 0 ? `Estimated saved: ${format(saved)} tokens` : ''
  ].filter(Boolean).join('\n');
...
export function formatFindResults(results) {
  return results.length
    ? results.map(row => `${row.file}:${row.line}  ${row.kind} ${row.name}  ${row.signature}`).join('\n')
    : 'No matching symbols found. Try `token-guard index` first, or search with a shorter query.';
}

function recordTargetedRead(projectRoot, result, meta = {}) {
  ...
  if (
    saved > 0 &&
    returned > 0 &&
    !String(result.kind || '').includes('not-found') &&
    result.kind !== 'diff-first-read'
  ) {
    appendEvent(projectRoot, {
      type: 'context_read_saved',
      file: result.file,
      kind: result.kind,
      method: meta.method || result.kind || 'read',
...
function diffRead(projectRoot, relPath, content, maxTokens) {
  ...
  if (!previous) {
    return {
      kind: 'diff-first-read',
      file: relPath,
      originalTokens: estimateTokens(content),
      returnedTokens: 0,
      text: 'No previous snapshot. Snapshot saved for future diff-only reads.'
    };
  }
  ...
  if (previous.hash === currentHash) {
    ...
    return {
      kind: 'diff-unchanged',
      file: relPath,
      originalTokens: saved,
      returnedTokens: 12,
      text: `Unchanged since ${previous.ts}. Full reread avoided.`
    };
  }
...
function readAroundText(projectRoot, relPath, content, lines, query, context, maxTokens) {
  ...
  if (idx < 0) {
    return {
      kind: 'around-not-found',
      file: relPath,
      requested: q,
      text: `Token Guard could not find text "${q}" in ${relPath}.`,
      originalTokens: estimateTokens(content),
      returnedTokens: 0
    };
  }
...
function findSymbolRange(lines, symbols, match, contextLines) {
  ...
  return {
    ...
    end: Math.min(lines.length, end + Math.floor(contextLines / 2))
  };
}

function expandBraceRange(lines, startLine, fallbackEnd) {
  let depth = 0;
  let seen = false;
  ...
  for (let i = startLine - 1; i < lines.length; i += 1) {
    ...
    if (seen && depth <= 0 && i > startLine - 1) {
      return i + 1;
    }

    if (i + 1 > fallbackEnd + 200) break;
  }
...
function makeLargeFilePreview(file, content, lines, maxTokens) {
  ...
  const text = `${addLineNumbers(head, 1)}
...
${addLineNumbers(tail, tailStart + 1)}`;
  ...
  return {
    ...
    originalTokens: estimateTokens(content),
    returnedTokens: estimateTokens(text),
    text: `Large file preview. Full read would cost ~${format(estimateTokens(content))} tokens.
...
function findSection(lines, query) {
  ...
  const sections = extractMarkdownSections(lines);
  const md = sections.find(section => section.title.toLowerCase().includes(q));

  if (md) {
    const next = sections.find(section => section.line > md.line && section.level <= md.level);
...
function shouldSkip(projectRoot, relPath, absolute, config) {
  ...
  if (relPath.startsWith('TokenGuard/') || relPath.startsWith('.token-guard/')) return true;
  if (relPath.includes('/node_modules/')) return true;
  if (isAlwaysAllowed(relPath, config) || isAlwaysGuarded(relPath, config)) return true;
...
function safeRead(file, maxBytes) {
  ...
  return fs.readFileSync(file, 'utf8').slice(0, Math.min(stat.size, maxBytes));
}
